...
switch($opcion){
    case 1:
        ...
        INSERT INTO cliente (nombre, cuit, telefono, mail, observaciones, direccionID, usuarioID) VALUES('$nombre', '$cuit', '$telefono', '$mail', '$observaciones', @direccionID, @usuarioID );
        INSERT INTO permisos (rol, usuarioID) VALUES ('cliente', @usuarioID);";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);       
        break;    
    case 2:
        $consulta = "UPDATE cliente SET nombre='$nombre', cuit='$cuit', telefono='$telefono', mail='$mail', observaciones='$observaciones' WHERE idCliente='$idCliente';
        UPDATE direccion SET calle='$calle', numero='$numero', localidad='$localidad' WHERE idDireccion = (SELECT d.idDireccion FROM cliente c INNER JOIN direccion d ON c.direccionID = d.idDireccion WHERE c.idCliente = '$idCliente');
        UPDATE usuarios SET usuario='$usuario' WHERE idUsuario = (SELECT u.idUsuario FROM cliente c INNER JOIN usuarios u ON c.usuarioID = u.idUsuario WHERE c.idCliente = '$idCliente')";      
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
